...
A `ConsumerProfile` names one harness/surface, evidence revision, native loader, conversion capability, resource behavior, and lifecycle adapter. CLI and desktop profiles remain distinct. A profile with missing runtime evidence returns `unverified`; a known incompatible semantic returns `unsupported`; neither activates a partial install.
