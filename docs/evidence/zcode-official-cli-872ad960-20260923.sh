...
run_in_home() {
  local probe_home=$1; shift
  HOME="$probe_home" ZCODE_STORAGE_DIR="$probe_home/.zcode" zcode "$@"
}
plugin_count() {
  local json=$1 id=$2
...
probe_home=$(new_home)
plugin="$probe_home/owned/demo"
resource="$probe_home/plgnz-resources/demo/fp/HTML-REPORT.md"
...
\$ARGUMENTS
EOF
mkdir -p "$probe_home/.zcode/cli"
printf '{"plugins":{"enabled":true,"dirs":["%s"]}}\n' "$plugin" > "$probe_home/.zcode/cli/config.json"
run_in_home "$probe_home" skills list --json > "$out/uos.skills.json"
run_in_home "$probe_home" commands inspect report --json > "$out/uos.command.json"

# Failed update retains an enabled prior install and its native cache record.
probe_home=$(new_home); market="$probe_home/failure-market"; write_market "$market" failure-market 1.0.0 one
run_in_home "$probe_home" plugins marketplace add "$market" > "$out/failure.marketplace-add.out"
run_in_home "$probe_home" plugins install demo@failure-market > "$out/failure.install.out"
printf '{"name":"failure-market","plugins":[{"name":"demo","source":"./plugins/missing"}]}\n' > "$market/marketplace.json"
run_in_home "$probe_home" plugins marketplace update failure-market > "$out/failure.marketplace-update.out"
set +e
run_in_home "$probe_home" plugins update demo@failure-market > "$out/failure.update.out" 2> "$out/failure.update.err"
status=$?
set -e
printf '%s\n' "$status" > "$out/failure.update.status"
[[ $status -ne 0 ]] || { echo "expected failed marketplace update" >&2; exit 1; }
run_in_home "$probe_home" plugins list --json > "$out/failure.after.json"
...
probe_home=$(new_home); market="$probe_home/same-version"; write_market "$market" same-version 1.0.0 one
run_in_home "$probe_home" plugins marketplace add "$market" > "$out/same.marketplace-add.out"
run_in_home "$probe_home" plugins install demo@same-version > "$out/same.install.out"
printf '%s\n' two > "$market/plugins/demo/skills/a/SKILL.md"
run_in_home "$probe_home" plugins marketplace update same-version > "$out/same.marketplace-update.out"
run_in_home "$probe_home" plugins update demo@same-version > "$out/same.plugin-update.out"
run_in_home "$probe_home" plugins list --json > "$out/same.after.json"
...
probe_home=$(new_home); market="$probe_home/buildmeta"; write_market "$market" buildmeta 1.0.0+plgnz.0123456789abcdef one
run_in_home "$probe_home" plugins marketplace add "$market" > "$out/buildmeta.marketplace-add.out"
run_in_home "$probe_home" plugins install demo@buildmeta > "$out/buildmeta.install.out"
run_in_home "$probe_home" plugins list --json > "$out/buildmeta.after.json"
...
probe_home=$(new_home); market="$probe_home/readd"; write_market "$market" readd 1.0.0 one
run_in_home "$probe_home" plugins marketplace add "$market" > "$out/readd.marketplace-add.out"
run_in_home "$probe_home" plugins install demo@readd > "$out/readd.first.out"
run_in_home "$probe_home" plugins install demo@readd > "$out/readd.second.out"
run_in_home "$probe_home" plugins list --json > "$out/readd.after.json"
